...
from platoon.issue_resolution.test_execution_reward.test_execution_reward import (
    compute_test_execution_reward,
)
...
def extract_patch_from_environment(
    ...
    if git_patch_result.exit_code != 0:
        logger.error(
            "git diff command failed with exit code %s and stderr: %s %s",
            git_patch_result.exit_code,
            git_patch_result.stderr,
            git_patch_result.stdout,
        )
...
class SWEBenchEnv(OpenHandsEnv):
    async def evaluate(self) -> tuple[float, dict]:
        ...
        _, test_execution_info = await compute_test_execution_reward(model_patch, instance)
        reward = float(test_execution_info.get("binary_reward", 0.0))
