...
> This file is an environment inventory only. Training does not read it, and it
> is not required to use a 32,768-token context window.
...
while `max_tokens` carries the prompt-plus-completion limit. The current legacy
YAML uses a 32,768-token total engine limit and a 2,048-token per-call completion
limit. AReaL's fallback `max_tokens` value is already 32,768, so this hotfix is
not required for that context size; it only matters when configuring a different
total engine limit on the affected AReaL revision.
