"""CodeScout rollouts backed by OpenHands running in Sailboxes."""
...
from platoon.codescout_sail.custom_tools.localization_finish import (
    LocalizationFinishTool,  # noqa: F401
)
from platoon.codescout_sail.env import CodeScoutEnv
from platoon.codescout_sail.tasks import (
...
def _sail_workspace(working_dir: str) -> SailWorkspace:
    """Create one isolated Sailbox for a CodeScout rollout."""
    return SailWorkspace(
        server_image=os.environ.get("CODESCOUT_AGENT_SERVER_IMAGE", EVAL_AGENT_SERVER_IMAGE),
        target_type="source",
        app_name=os.environ.get("SAIL_APP_NAME", "codescout-agent-server"),
        sailbox_name=_optional_env("SAILBOX_NAME"),
        working_dir=working_dir,
        image_architecture=_optional_env("SAIL_IMAGE_ARCHITECTURE"),
        image_build_timeout=int(os.environ.get("SAIL_IMAGE_BUILD_TIMEOUT", "1800")),
        force_image_build=_bool_env("SAIL_FORCE_IMAGE_BUILD"),
        create_timeout=int(os.environ.get("SAIL_CREATE_TIMEOUT", "600")),
        startup_timeout=float(os.environ.get("SAIL_STARTUP_TIMEOUT", "600")),
        size=os.environ.get("SAILBOX_SIZE", "s"),
        memory_limit_gib=int(os.environ.get("SAILBOX_MEMORY_GIB", "2")),
        disk_limit_gib=int(os.environ.get("SAILBOX_DISK_GIB", "16")),
        private=_bool_env("SAILBOX_PRIVATE"),
        inbound_allowlist=_optional_csv_env("SAILBOX_INBOUND_ALLOWLIST"),
        workspace_owner=_optional_env("SAIL_WORKSPACE_OWNER") or "10001:10001",
        expected_server_git_sha=_optional_env("SAIL_EXPECTED_SERVER_GIT_SHA"),
    )


def prepare_workspace(instance: dict) -> tuple[Path, SailWorkspace]:
    """Start a Sailbox and clone the task repository into it."""
    ...
    for attempt in range(1, NUM_RETRIES_SANDBOX_START + 1):
        workspace: SailWorkspace | None = None
        try:
            workspace = _sail_workspace(str(instance_path))
            ...
            if attempt < NUM_RETRIES_SANDBOX_START:
                logger.warning(
                    "Sail workspace setup attempt %s/%s failed for %s: %s",
                    attempt,
                    NUM_RETRIES_SANDBOX_START,
                    instance_id,
                    error,
    ...
    raise RuntimeError(f"Error preparing Sail workspace for instance {instance_id}: {last_error}")
...
async def run_rollout(task: Task, config: RolloutConfig) -> dict | TrajectoryCollection:
    """Run one CodeScout trajectory and destroy its Sailbox afterward."""
    ...
    try:
        if config.verbose:
            print(f"[run_rollout] Starting Sail rollout for task {task.id}", flush=True)
        ...
        if env is not None:
            try:
                ...
                logger.warning("Sail cleanup failed for task %s: %s", task.id, cleanup_error)
