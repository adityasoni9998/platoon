...
def prepare_workspace(instance: dict):
    ...
    for i in range(NUM_RETRIES):
        try:
            ...
            _run(
                f"git clone --depth 1 --branch {instance_id} --single-branch "
                f"https://github.com/{repo_name}.git {str(instance_path)}",
                timeout=120.0,
            )
            break
        except Exception as e:
            if i == NUM_RETRIES - 1:
                raise RuntimeError(f"Error preparing workspace for instance {instance_id}: {str(e)}")
            try:
                workspace.cleanup()
