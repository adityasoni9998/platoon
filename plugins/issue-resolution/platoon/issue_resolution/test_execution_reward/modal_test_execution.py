...
REMOTE_EVAL_SCRIPT_PATH = "/eval.sh"
...
swerebench_image = modal.Image.debian_slim().apt_install("git").pip_install("git+https://github.com/adityasoni9998/SWE-rebench-harness.git", "tenacity")
...
class ModalSandboxRuntime:
    ...
    def get_instance_image(test_spec: TestSpec) -> modal.Image:
        image_name = test_spec.image_name
        if image_name is None:
            instance_id = test_spec.instance_id
            image_name = f"swerebench/sweb.eval.x86_64.{instance_id.lower()}"
            image_name = image_name.replace("__", "_1776_")
        image = modal.Image.from_registry(image_name).add_local_file(
